...
class Renderer:
    ...
    def render_inventory_screen(self, player: Player, selected_index: int, dungeon_level: int):
        ...
        try:
            # Draw border
            self.stdscr.addch(0, 0, tl)
            self.stdscr.addch(0, max_x - 2, tr)
            self.stdscr.addch(max_y - 2, 0, bl)
            self.stdscr.addch(max_y - 2, max_x - 2, br)

            for x in range(1, max_x - 2):
                self.stdscr.addch(0, x, h_char)
                self.stdscr.addch(max_y - 2, x, h_char)

            for y in range(1, max_y - 2):
                self.stdscr.addch(y, 0, v_char)
                self.stdscr.addch(y, max_x - 2, v_char)
            ...
            self.stdscr.addstr(2, 3, capacity_str)

            # Column headers
            self.stdscr.addstr(4, 3, "ITEM", curses.A_BOLD if curses.has_colors() else curses.A_NORMAL)
            self.stdscr.addstr(4, 35, "TYPE", curses.A_BOLD if curses.has_colors() else curses.A_NORMAL)
            self.stdscr.addstr(4, 50, "DESCRIPTION", curses.A_BOLD if curses.has_colors() else curses.A_NORMAL)

            # Divider line
            self.stdscr.addstr(5, 3, "─" * (max_x - 8) if use_unicode else "-" * (max_x - 8))

            # Item list
            inventory = player.inventory
            if inventory.is_empty():
                empty_msg = "Your inventory is empty. Pick up items by walking over them."
                self.stdscr.addstr(7, 3, empty_msg)
            else:
                for i, item in enumerate(inventory.items):
                    ...
                    if i == selected_index:
                        ...
                        if curses.has_colors():
                            self.stdscr.addstr(y, 2, indicator, curses.color_pair(2) | curses.A_BOLD)
                        else:
                            self.stdscr.addstr(y, 2, indicator, curses.A_BOLD)

                    # Item name with rarity color
                    name_str = f"{item.symbol} {item.name}"
                    if curses.has_colors() and item.rarity:
                        color_pair = ITEM_RARITY_COLORS.get(item.rarity, 1)
                        self.stdscr.addstr(y, 4, name_str, curses.color_pair(color_pair))
                    else:
                        self.stdscr.addstr(y, 4, name_str)

                    # Item type
                    type_str = item.item_type.name.replace("_", " ").title()
                    self.stdscr.addstr(y, 35, type_str[:12])

                    # Description
                    desc_str = item.description[:max_x - 55] if len(item.description) > max_x - 55 else item.description
                    self.stdscr.addstr(y, 50, desc_str)
            ...
            if not inventory.is_empty() and 0 <= selected_index < len(inventory.items):
                ...
                self.stdscr.addstr(details_y, 3, "─" * (max_x - 8) if use_unicode else "-" * (max_x - 8))
                self.stdscr.addstr(details_y + 1, 3, "SELECTED: ", curses.A_BOLD)

                if curses.has_colors() and item.rarity:
                    color_pair = ITEM_RARITY_COLORS.get(item.rarity, 1)
                    self.stdscr.addstr(details_y + 1, 13, item.name, curses.color_pair(color_pair) | curses.A_BOLD)
                else:
                    self.stdscr.addstr(details_y + 1, 13, item.name, curses.A_BOLD)
            ...
            if curses.has_colors():
                ...
                self.stdscr.addstr(controls_y, controls_x, controls)
    ...
    def render_character_screen(self, player: Player, dungeon_level: int):
        ...
        try:
            ...
            self.stdscr.addstr(3, 5, "ADVENTURER", curses.A_BOLD)
            if curses.has_colors():
                self.stdscr.addstr(3, 17, f" {player.symbol}", curses.color_pair(2) | curses.A_BOLD)

            # Stats section
            self.stdscr.addstr(5, 5, "═" * 30 if use_unicode else "=" * 30)
            self.stdscr.addstr(6, 5, "STATS", curses.A_BOLD)
            self.stdscr.addstr(5, 40, "═" * 30 if use_unicode else "=" * 30)
            self.stdscr.addstr(6, 40, "PROGRESS", curses.A_BOLD)

            # Left column - Combat stats
            y = 8
            self.stdscr.addstr(y, 5, f"Level:        {player.level}")
            y += 1

            # Health with bar
            health_pct = player.health / player.max_health if player.max_health > 0 else 0
            health_bar = self._render_bar(player.health, player.max_health, 15)
            health_color = curses.color_pair(4) if health_pct > 0.5 else (curses.color_pair(2) if health_pct > 0.25 else curses.color_pair(3))
            self.stdscr.addstr(y, 5, f"Health:       ")
            if curses.has_colors():
                self.stdscr.addstr(y, 19, health_bar, health_color)
            else:
                self.stdscr.addstr(y, 19, health_bar)
            self.stdscr.addstr(y, 35, f"{player.health}/{player.max_health}")
            y += 1
            ...
            y += 1
            self.stdscr.addstr(y, 5, f"Dungeon Lv:   {dungeon_level}")

            # Right column - Progress stats
            y = 8
            self.stdscr.addstr(y, 40, f"Kills:        {player.kills}")
            y += 1

            # XP with bar
            xp_in_level = player.xp - (player.xp_to_next_level - player.level * 30)
            xp_needed = player.level * 30
            xp_bar = self._render_bar(xp_in_level, xp_needed, 15)
            self.stdscr.addstr(y, 40, f"Experience:   ")
            if curses.has_colors():
                self.stdscr.addstr(y, 54, xp_bar, curses.color_pair(5))
            else:
                self.stdscr.addstr(y, 54, xp_bar)
            self.stdscr.addstr(y, 70, f"{xp_in_level}/{xp_needed}")
            y += 1

            self.stdscr.addstr(y, 40, f"Total XP:     {player.xp}")
            y += 1
            ...
            y += 1
            self.stdscr.addstr(y, 5, f"Items: {len(player.inventory.items)}/{player.inventory.max_size}")
    ...
    def render_help_screen(self):
        ...
        try:
            ...
            # Movement section
            self.stdscr.addstr(y, 5, "MOVEMENT", curses.A_BOLD)
            y += 1
            self.stdscr.addstr(y, 5, "Arrow Keys / WASD    Move in direction")
            y += 1
            self.stdscr.addstr(y, 5, "Bump into enemy      Attack")
            y += 2

            # Screens section
            self.stdscr.addstr(y, 5, "SCREENS", curses.A_BOLD)
            y += 1
            self.stdscr.addstr(y, 5, "I                    Open Inventory")
            y += 1
            self.stdscr.addstr(y, 5, "C                    Character Stats")
            y += 1
            self.stdscr.addstr(y, 5, "?                    This Help Screen")
            y += 2

            # Items section
            self.stdscr.addstr(y, 5, "ITEMS", curses.A_BOLD)
            y += 1
            self.stdscr.addstr(y, 5, "1-3                  Quick use item (from sidebar)")
            y += 1
            self.stdscr.addstr(y, 5, "Walk over item       Auto pickup")
            y += 2

            # Game section
            self.stdscr.addstr(y, 5, "GAME", curses.A_BOLD)
            y += 1
            self.stdscr.addstr(y, 5, ">                    Stairs down (descend)")
            y += 1
            self.stdscr.addstr(y, 5, "Q                    Save and Quit")
            y += 2

            # Symbols section
            self.stdscr.addstr(y, 5, "SYMBOLS", curses.A_BOLD)
            y += 1
            if curses.has_colors():
                self.stdscr.addstr(y, 5, "@", curses.color_pair(2))
                self.stdscr.addstr(y, 7, "You")
                self.stdscr.addstr(y, 20, "g s o", curses.color_pair(3))
                self.stdscr.addstr(y, 26, "Enemies (goblin, skeleton, orc)")
            else:
                self.stdscr.addstr(y, 5, "@ You     g s o Enemies")
            y += 1
            if curses.has_colors():
                self.stdscr.addstr(y, 5, "!", curses.color_pair(5))
                self.stdscr.addstr(y, 7, "Potion")
                self.stdscr.addstr(y, 20, "?", curses.color_pair(5))
                self.stdscr.addstr(y, 22, "Scroll")
                self.stdscr.addstr(y, 35, ">", curses.color_pair(1))
                self.stdscr.addstr(y, 37, "Stairs down")
            else:
                self.stdscr.addstr(y, 5, "! Potion  ? Scroll  > Stairs")
